...
export class CreateTaskManagementComponent implements OnInit, AfterViewInit {
      ...
      console.log('Selected value:', this.selectedBusinessId);
        ...
        // Retrieve customer information based on the selected business ID
          if (this.selectedBusinessId) {
            this.http.get(`http://localhost:8086/api/Business/BusinessInfo/${this.selectedBusinessId}`)
              .subscribe(
                (response: any) => {
  ...
  onSubmit() {
    ...
    if (this.taskForm.valid) {
      ...
      this.http.post('http://localhost:8086/api/CallLog/CreateTask', formData)
        .subscribe(
          response => {
  ...
  getGeneratedTicketId() {
    this.http.get<string>('http://localhost:8086/api/CallLog/GenerateTicketId')
      .subscribe((id: string) => {
  ...
  loadAllBusiness() {
    this.http.get("http://localhost:8086/api/Business/BusinessInfo")
      .subscribe(
        (res: any) => {
  ...
  async loadAllTasks() {
    try {
      const res = await this.http.get("http://localhost:8086/api/CallLog/GetAllCallLogTaskInfo").toPromise();
