...
export class TaskManagementComponent implements OnInit {
  ...
  loadAllTasks(): void {
    this.http.get("http://localhost:8086/api/CallLog/GetAllCallLogTaskInfo").subscribe(
      (res: any) => {
        this.taskArray = res;
        console.log(res);
      },
  ...
  //   this.http.delete(`http://localhost:8086/api/CallLog/DeleteTask/${task.ticketId}`)
  //   .subscribe(
  //     (response: any) => {
  ...
  updateTaskStatus(task: any): void {
    const url = `http://localhost:8086/api/tasks/${task.id}`;
