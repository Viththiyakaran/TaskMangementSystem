...
export class EditTaskManagementComponent implements OnInit, AfterViewInit {
  ...
  loadNotesById(taskId: string) {
    const url = `http://localhost:8086/api/LogNote/GetCallLogNotesInfo/${taskId}`;
  ...
  loadTaskDetails(taskId: string) {
    const url = `http://localhost:8086/api/CallLog/GetCallLogTaskInfoById/${taskId}`;
    ...
    setTimeout(() => {
      ...
      $('#businessId').on('change', (event) => {
        ...
        if (this.selectedBusinessId) {
          this.http
            .get(
              `http://localhost:8086/api/Business/BusinessInfo/${this.selectedBusinessId}`
            )
            .subscribe(
              (response: any) => {
  ...
  onSubmitNote() {
    ...
    this.http.post('http://localhost:8086/api/LogNote/CreateNote', formData).subscribe(
      (response: any) => {
  ...
  onSubmit() {
    ...
    if (this.taskForm.valid) {
      ...
      if(confirmation)
      {
          const url = `http://localhost:8086/api/CallLog/PutCallLogTaskInfo/${taskId}`;
  ...
  loadAllBusiness() {
    this.http
      .get('http://localhost:8086/api/Business/BusinessInfo')
      .subscribe(
        (res: any) => {
  ...
  async loadAllTasks() {
    try {
      const res = await this.http
        .get('http://localhost:8086/api/CallLog/GetAllCallLogTaskInfo')
        .toPromise();
  ...
  loadAllUsers() {
    this.http.get("http://localhost:8086/api/User/GetAllUserInfo")
      .subscribe(
        (res: any) => {
          this.userArray = res;
          this.userArray2 = res;
